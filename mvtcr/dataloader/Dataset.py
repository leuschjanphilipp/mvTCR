...
class JointDataset(torch.utils.data.Dataset):
    ...
    def _init_optional_tensor(self, data, dtype=torch.LongTensor):
        return dtype(data) if data is not None else None
    ...
    def __getitem__(self, idx):
        return {
            "tcr": self.tcr_data[idx],
            "tcr_length": self.tcr_length[idx], 
            "rna": self.rna_data[idx], 
            "vdj": self.vdj_data[idx] if self.vdj_data is not None else torch.tensor([]),
            "citeseq": self.citeseq_data[idx] if self.citeseq_data is not None else torch.tensor([]), 
            "metadata": self.metadata[idx] if self.metadata is not None else torch.tensor([]),
            "labels": self.labels[idx] if self.labels is not None else torch.tensor([]), 
            "conditional": self.conditional[idx] if self.conditional is not None else torch.tensor([])
        }
